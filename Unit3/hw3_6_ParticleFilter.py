...
class robot:
    ...
    def move(self, motion):
        steering_angle_rel_to_car = motion[0] * 1.0
        distance_moved = motion[1] * 1.0

        # Add steering and distance noise
        steering_angle_rel_to_car2 = random.gauss(steering_angle_rel_to_car, self.steering_noise)
        distance_moved2 = random.gauss(distance_moved, self.distance_noise)

        turning_angle = (distance_moved2 / self.length) * tan(steering_angle_rel_to_car2)

        new_robot = robot(self.length)
        new_robot.set_noise(self.bearing_noise, self.steering_noise, self.distance_noise)

        if abs(turning_angle) < 0.001:                                # Going straight
            new_x = self.x + distance_moved2 * cos(self.orientation)
            new_y = self.y + distance_moved2 * sin(self.orientation)
            new_orientation = self.orientation + turning_angle
            new_robot.set(new_x, new_y, new_orientation)
        else:                                                         # Turning
            turning_radius = distance_moved2 / turning_angle

            cx = self.x - sin(self.orientation) * turning_radius
            cy = self.y + cos(self.orientation) * turning_radius

            new_x = cx + sin(self.orientation + turning_angle) * turning_radius
            new_y = cy - cos(self.orientation + turning_angle) * turning_radius
            new_orientation = (self.orientation + turning_angle) % (2.0 * pi)
            new_robot.set(new_x, new_y, new_orientation)
    ...
    def sense(self, add_noise=1): #do not change the name of this function
        Z = []
        for (land_y, land_x) in landmarks:
            bearing = atan2(land_y - self.y, land_x - self.x) - self.orientation
