...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        turning_angle = (distance_moved / self.length) * tan(steering_angle_rel_to_car)
        ...
        if abs(turning_angle) < 0.001:                                # Going straight
            new_x = self.x + distance_moved * cos(self.orientation)
            new_y = self.y + distance_moved * sin(self.orientation)
            new_orientation = self.orientation + turning_angle
            new_robot.set(new_x, new_y, new_orientation)
        else:                                                         # Turning
            turning_radius = distance_moved / turning_angle
